...
% last updated 1/10/17 - ahamilos

% updated for use with operant and hybrid in same session
% NEED TO FIX: You do a dice roll for pav vs op even when in the op only condition. You could fix this by making percent op 100
% but there should be a way to detect if it was all operant...
% For now, assume V4 is for NON-MIXED pav-op - ie it's all operant. So look to see if operant param is 1
...
for ievent = 1:length(events)
    ...
    if trial_index > numtrials
        disp(' reached end of trials ')
    elseif all_trials_parameters(trial_index, 2) == 1 % hybrid trial
        hybrid_by_trial(trial_index) = 1;
    elseif all_trials_parameters(trial_index, 3) == 1 && all_trials_parameters(trial_index, 4) == 0 % pavlovian trial
        pav_or_op_by_trial(trial_index) = 0;
    elseif all_trials_parameters(trial_index, 3) == 0 && all_trials_parameters(trial_index, 4) == 1 % operant trial    
        pav_or_op_by_trial(trial_index) = 1;
    
    elseif events(ievent, 1) == 14 % 0 for pavlovian mixed
        if all_trials_parameters(trial_index, 3) == 1 && all_trials_parameters(trial_index, 4) == 1 % mixed trial
            pav_or_op_by_trial(trial_index) = 0;
            disp(['mixed pavlovian @ ', trial_index])
        end
    elseif events(ievent, 1) == 15 % 0 for operant mixed
        if all_trials_parameters(trial_index, 3) == 1 && all_trials_parameters(trial_index, 4) == 1 % mixed trial
            pav_or_op_by_trial(trial_index) = 1;
            disp(['mixed operant @ ', trial_index])
        end 
    end 
end
...
for ievent = 1:length(events)
    ...
    if trial_index <= numtrials
        ...
        if events(ievent, 1) == 9             % if lick detected
            ...
            if first_post_window_lick == false && events(ievent, 2) - cue_on_time_by_trial(trial_index) - start_time > time_max_interval && first_abort_window_lick == false && first_reward_lick == false;
                times_1st_late_lick(length(times_1st_late_lick)+1) = events(ievent, 2) - cue_on_time_by_trial(trial_index) - start_time;
                first_post_window_lick = true;
                allfirstlicks(length(allfirstlicks)+1) = events(ievent, 2) - cue_on_time_by_trial(trial_index) - start_time;
                if pav_or_op_by_trial(trial_index) == 0   % for a pavlovian trial...
...
plot([time_target,time_target], [0, 200], 'r-', 'LineWidth', 3, 'DisplayName', 'Target');
plot([time_abort_min,time_abort_min], [0, 200], 'b--', 'LineWidth', 3, 'DisplayName', 'Early Abort Min');
% plot([time_abort_max,time_abort_max], [0, 100], 'b--', 'LineWidth', 3);
plot([time_min_interval,time_min_interval], [0, 200], 'g--', 'LineWidth', 3, 'DisplayName', 'Reward Min');
% plot([time_max_interval,time_max_interval], [0, 100], 'g--', 'LineWidth', 3);
plot([time_trial_duration,time_trial_duration], [0, 200], 'c--', 'LineWidth', 3, 'DisplayName', 'Trial End');
ylim([0,70]);
% For normal plotting of all timepoints:
% xlim([min(min(licks_by_trial_relative_to_cue)) - 500, max(max(licks_by_trial_relative_to_cue)) + 500]);
% For plotting of Mr 15's data:
xlim([0, 7000]);
ylabel('number of licks');
xlabel('time relative to cue on (ms)');
title('Hybrid Combined First licks');
...
%% Single combined operant 1st licks fig

figure 
% histogram([op_times_1st_post_cue_lick, op_times_1st_lick_no_lick_period, pre_op_times_1st_lick_reward, op_times_1st_lick_reward, op_times_1st_late_lick],...
%     20);
% hold on
% histogram([op_times_1st_post_cue_lick, op_times_1st_lick_no_lick_period, pre_op_times_1st_lick_reward, op_times_1st_lick_reward, op_times_1st_late_lick],...
%     50);
...
% plot([0,0], [0, 100], 'g-'); 
% 
% plot([time_target,time_target], [0, 300], 'r-');
% plot([time_abort_min,time_abort_min], [0, 300], 'b--');
% plot([time_abort_max,time_abort_max], [0, 300], 'b--');
% plot([time_min_interval,time_min_interval], [0, 300], 'g--');
% plot([time_max_interval,time_max_interval], [0, 300], 'g--');
% plot([time_trial_duration,time_trial_duration], [0, 300], 'c--');
