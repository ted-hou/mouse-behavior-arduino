		...
		function OnStateChanged_VisualStim(obj, ~, ~)
			switch upper(obj.Arduino.StateNames{obj.Arduino.State})
				...
				case 'STIM_ON'
                    ...
                    maxTrialLength 	= obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'MAX_TRIAL_LENGTH')); % in seconds
					...
					if obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'TIMING')) == 1 % is timing trial
                        ...
                        while trialLength > maxTrialLength
                            trialLength = exprnd(mu) + minTrialLength; 
                        end
                    end
...
% 					end
					...
					obj.Rsc.Circle.UserData.Thetas 						= thetas;
					obj.Rsc.Circle.UserData.ThetaIndex					= thetaIndex0;
					obj.Rsc.Circle.UserData.Direction 					= 1;
					obj.Rsc.Circle.UserData.Speed						= speed;
					obj.Rsc.Circle.UserData.IsAlphaReached				= false;
					obj.Rsc.Circle.UserData.IsOmegaReached				= false;
					...
					obj.Rsc.CircleRefreshTimer.Period					= 1/speed;
					obj.Rsc.CircleRefreshTimer.TimerFcn					= {@obj.OnCircleRefresh, obj.Rsc.Circle};

					% Circle flashing during Circle_stat only
					obj.Rsc.Circle2.UserData.Thetas 					= thetas;
					obj.Rsc.Circle2.UserData.ThetaIndex					= thetaIndex0;
					obj.Rsc.Circle2.UserData.Speed						= speed;
					obj.Rsc.Circle2.FaceColor 							= [1 1 1];
				...
				case 'REWARD'
					...
					if obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'OPERANT_TURN')) == 1
						obj.Rsc.Circle.UserData.IsListOfThetasTruncated = true;
						obj.Rsc.Circle.UserData.Thetas = obj.Rsc.Circle.UserData.Thetas(1:obj.Rsc.Circle.UserData.ThetaIndex);
					end
		...
		function OnCircleRefresh(obj, t, ~, hCircle)
			try
				nextThetaIndex = obj.Rsc.Circle.UserData.ThetaIndex + hCircle.UserData.Direction;
				...
				if nextThetaIndex > 0
					% Alpha
					if (~hCircle.UserData.IsAlphaReached && nextThetaIndex <= length(obj.Rsc.Circle.UserData.Thetas) && hCircle.UserData.Direction > 0 && abs(obj.Rsc.Circle.UserData.Thetas(end) - obj.Rsc.Circle.UserData.Thetas(nextThetaIndex)) < windowDuration*speed*spatialFrequency)
						obj.Arduino.SendMessage('A');
						hCircle.UserData.IsAlphaReached = true;
					end
					% Turning point
					if (~hCircle.UserData.IsTurningPointReached && hCircle.UserData.Direction > 0 && nextThetaIndex > length(obj.Rsc.Circle.UserData.Thetas)) % turn when reach end of list of thetas
						% If we need to stop but haven't yet
						if obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'NUM_HOPS')) > 0 && ~obj.Rsc.Circle.UserData.IsCirclePaused && ~obj.Rsc.Circle.UserData.IsListOfThetasTruncated
							% Stop the Circle for a few hops, but send Turning point later
							if obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'REACTIVE')) == 0
								obj.Rsc.Circle.UserData.Thetas = [obj.Rsc.Circle.UserData.Thetas, repmat(obj.Rsc.Circle.UserData.Thetas(end), 1, obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'NUM_HOPS')))];
								obj.Rsc.Circle.UserData.IsCirclePaused = true;
							% Stop the Circle for a few hops, but send Turning point now
							elseif obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'REACTIVE')) == 1
								obj.Rsc.Circle.UserData.Thetas = [obj.Rsc.Circle.UserData.Thetas, repmat(obj.Rsc.Circle.UserData.Thetas(end), 1, obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'NUM_HOPS')))];
								obj.Rsc.Circle.UserData.IsCirclePaused = true;
								obj.Arduino.SendMessage('T');
								% For training reactive task, make Circle visible at turning point
								if (obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'FADE_STIM')) == 1)
									set(obj.Rsc.Circle, 'Visible', 'on');
								end
							end
						% Reached the end of the list for real this time I promise
						else
							hCircle.UserData.Direction = -1;
							hCircle.UserData.IsTurningPointReached = true;
							...
							if obj.Arduino.ParamValues(ismember(obj.Arduino.ParamNames, 'REACTIVE')) == 0
								...
								if ~strcmpi(obj.Arduino.StateNames{obj.Arduino.State}, 'REWARD')
									obj.Rsc.Circle.UserData.Thetas = obj.Rsc.Circle.UserData.Thetas(end):spatialFrequency:(obj.Rsc.Circle.UserData.Thetas(end) + spatialFrequency*(length(obj.Rsc.Circle.UserData.Thetas) - 1));
									obj.Rsc.Circle.UserData.Thetas = flip(obj.Rsc.Circle.UserData.Thetas);
								end
				...
				% If OmegaToITI interval is too long and bar in going in reverse, the list of thetas might not be long enough (nextThetaIndex <= 0)
					...
					nextTheta = obj.Rsc.Circle.UserData.Thetas(1) + mode(diff(obj.Rsc.Circle.UserData.Thetas))*(nextThetaIndex - 1);
				end
				...
				if (~hCircle.UserData.IsAlphaReached)
					obj.Arduino.SendMessage('A');
					hCircle.UserData.IsAlphaReached = true;
				end
				if (~hCircle.UserData.IsTurningPointReached)
					obj.Arduino.SendMessage('T');
					hCircle.UserData.IsTurningPointReached = true;
				end
				if (~hCircle.UserData.IsOmegaReached)
	...
	methods (Static)
		...
		function varargout = movingCircle(theta, varargin)
			p = inputParser;
		...
		function varargout = stationaryCircle(theta, varargin)
			p = inputParser;
			addRequired(p, 'Theta', @isnumeric);
			addParameter(p, 'Ax', []);
			addParameter(p, 'Circle', []);
