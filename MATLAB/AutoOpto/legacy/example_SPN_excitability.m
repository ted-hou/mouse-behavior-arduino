%%
% Optrode B along AP, facing right (lateral) of mouse
...
exp.validate(validationDelay=[8, 8])
exp.save();
%% Or load calibration
...
%% Give juice once every 4 stim trains

% 25uW, 4 trains per pos
for i = 1:4
    for iMirrorPos = 1:2
...
%% Give juice a lot, might do some manuals:
doReward(exp);
...
%%
exp.runStimTrain(iMirrorPos, 2, 1, nPulses=20, pulseWidth=1, ipi=1, preTrainDelay=5, postTrainDelay=1);
%%
exp.runStimTrain(iMirrorPos, 1, 1, nPulses=20, pulseWidth=1, ipi=1, preTrainDelay=5, postTrainDelay=1);
