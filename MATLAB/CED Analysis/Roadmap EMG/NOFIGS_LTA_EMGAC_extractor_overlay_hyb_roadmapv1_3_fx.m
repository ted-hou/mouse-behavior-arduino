function [rxn_signal_lick_triggered_trials,...
    ...
    pav_signal_lick_triggered_trials,...
    ...
    N_pav_signal,...
    N_ITI_signal,...
    time_array] = NOFIGS_LTA_EMGAC_extractor_overlay_hyb_roadmap1_3_fx(signal_vbt,num_trials,f_ex_lick_pavlovian,f_ex_lick_rxn,f_ex_lick_operant_no_rew,f_ex_lick_operant_rew,f_ex_lick_ITI, signalname)
...
% Modified ahamilos 12-5-17 (from figures + version)
% 
% 
% SMOOTH = 50 ms gauss
% 
...
% Pavlovian Licks (comment out for op only)--------------------------------------------------------------------------------------------------
   ...
   %% Variables to save:
    pav_signal_lick_triggered_trials = signal_lick_triggered_trials;
  
    pav_signal_lick_triggered_ave_ignore_NaN = signal_lick_triggered_ave_ignore_NaN;
    % %% The plots:---------------------------------------------------------------------------
    %     figure
    %     ax_pav = subplot(1,1,1) %***********************
    %     axisarray(1) = ax_pav;
    %     plot([0,0], [1,2])
    %     hold on
    %     plot(time_array, gausssmooth(pav_signal_lick_triggered_ave_ignore_NaN, smoothwin, 'gauss'), 'linewidth', 3)
    %     xlim(xlimits)
    %     title(title_signal)
...
%% Overlays (not normalized)
    ...
    N_pav_signal = pav_signal_lick_triggered_ave_ignore_NaN;
    ...
    % plot(time_array(pos1:pos2),N_pav_signal(pos1:pos2), 'linewidth', 3)
