...
% 
% 	created       7-31-17 ahamilos (from LTA_time_binner_hyb_roadmapv1.m)
% 	last modified 10-3-17 ahamilos
%
% UPDATE LOG
% 	10-3-17: Updated smooth, for movement controls
...
dataSetPavDLS = pav_DLS_lick_triggered_trials;
...
dataSetPavSNc = pav_SNc_lick_triggered_trials;
...
time_bound_2_rew = (cue_on_time + 5100)/1000;
time_bound_1_pav = (cue_on_time + 5100)/1000; 
time_bound_2_pav = (cue_on_time + 7000)/1000;
...
%% EARLY CASE:---------------------------------------------
	...
	% Add trial numbers to 2nd row to keep track of trial positions after sorting:
	rxn_times_with_trial_markers = f_licks_early;
...
%% PAV CASE---------------------------------------------
	DLS_vbt = dataSetPavDLS;
	SNc_vbt = dataSetPavSNc;
	time_bound_1 = time_bound_1_pav;
	time_bound_2 = time_bound_2_pav;

	% Add trial numbers to 2nd row to keep track of trial positions after sorting:
	rxn_times_with_trial_markers = f_licks_pav;
	...
	xlim(xwin)
	ylim([-1,1])
	title('DLS Pavlovian LTA Binned Averages');
	xlabel('time (ms)')
	ylabel('signal')
	...
	xlim(xwin)
	ylim([-1,1])
	title('SNc Pavlovian LTA Binned Averages');
	xlabel('time (ms)')
	ylabel('signal')
