% Roadmap TIME BINNER FOR HYBRID, 500ms-----------------------------------------------------------------------
% 
% 10-3-17 update: Updated for movement controls, fixed smooth windows, etc
% 8-10-17 update: Updated legend config
...
%       2. put_data_into_trials_aligned_to_cue_on_fx = EMG_ex_values_by_trial, X_ex_values_by_trial
...
%% REWARDED OPERANT CASE------------------------------------------------------------------------------------------
	...
	time_bound_2 = (cue_on_time + target_time + 100)/1000; % this is 5100 post cue ** error corrected 8-7-17
...
%% PAVLOVIAN CASE-------------------------------------------------------------------------------------------------
	...
	ylim([-1,1])
	xlim([0,cue_on_time + total_time])
	title('CTA Pav - X binned averages');
	xlabel('time (ms)')
	ylabel('signal')
	...
	ylim([-1,1])
	xlim([0,cue_on_time + total_time])
	title('CTA Pav - Y binned averages'); % *********************
	xlabel('time (ms)')
	ylabel('signal')
	...
	ylim([-1,1])
	xlim([0,cue_on_time + total_time])
	title('CTA Pav - Z binned averages'); % *********************
	xlabel('time (ms)')
	ylabel('signal')
	...
	ylim([-1,1])
	xlim([0,cue_on_time + total_time])
	title('CTA Pav - EMG binned averages');
	xlabel('time (ms)')
	ylabel('signal')
