function [] = LTA_photom_time_binner_op_roadmapv1_3_fx(time_array,...
												Hz,...
												dataSetRxnsignal,...
												dataSetEarlysignal,...
												dataSetRewsignal,...
...
% 	last modified 10-27-17 ahamilos (from LTA_time_binner_op_roadmapv1.m)
% 
% 	Dependencies:
% 		1. lick_triggered_ave_allplots_fx.m
% 
%  10-28-17: Modified to handle any sampling rate input
...
% dataSetITIsignal = ITI_signal_lick_triggered_trials;
	...
	time_bound_2_rew = (cue_on_time + 7000*sample_scaling_factor)/1000;
...
%% RXN CASE:---------------------------------------------
	...
	for ibins = 1:nbins
		plot(time_array(pos1:pos2), gausssmooth(signal_bin_aves{ibins}(pos1:pos2), smoothwindow, 'gauss'), 'linewidth', 3);
...
%% EARLY CASE:---------------------------------------------
	...
	% Add trial numbers to 2nd row to keep track of trial positions after sorting:
	early_times_with_trial_markers = f_licks_early*sample_scaling_factor;
	...
	xlim(xwin)
	ylim([-1,1])
	title([signalname, 'signal Early LTA Binned Averages']);
	xlabel('time (ms)')
	ylabel('signal')
