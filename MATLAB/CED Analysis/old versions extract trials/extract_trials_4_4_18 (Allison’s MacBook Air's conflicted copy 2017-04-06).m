...
for i_trial = 0:length(trial_start_times - 2)
	...
	for i_time = 1:18500 % fill the array from the front
		...
		if pastcue && ~dontupdate
			DLS_post_cue_times_by_trial(i_trial, time_markers(postcue_position)) = DLS_times_trimmed(DLS_position);
			DLS_post_cue_values_by_trial(i_trial, time_markers(postcue_position)) = DLS_values_trimmed(DLS_position);
			DLS_position = DLS_position + 1;
			postcue_position = postcue_position+1;
        elseif dontupdate
        % do nothing
