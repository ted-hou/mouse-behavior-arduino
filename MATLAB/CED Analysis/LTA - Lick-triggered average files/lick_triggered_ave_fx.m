...
%                                              num_trials,...
%                                              cue_position)
% --------------OBSOLETE! USE lick_triggered_ave_allplots_fx (7-27-17)-------------
% 
...
%       7-27-17: found error - should not fill in right_of_and_lick >0, should be > backfilltime
%
%
...
%% Debugging History---------------------------------------------------
    ...
    dummy_licks = [.006, .008, -.01, .003] + 0.01; % including the cue position-1, 10 (the -.01 mimics the zero of a non-lick in f_lick arrays)
...
%% DLS Plots---------------------------------------------------------------------------
    ...
    for i_trial = 1:num_trials
        ...
        if lick_positions(i_trial) > backfilltime % only positive lick positions should be considered, all negative are artifacts of there not being a recorded first lick on that trial
