...
trials_with_rxn, trials_with_train, trials_with_pav, trials_with_ITI, all_first_licks] = first_lick_grabber(lick_times_by_trial, num_trials)

% 7-21-17 VERSION IS OBSOLETE, USE NEWER!!!!!!!!!!!
% 
...
rxn_train_abort_range = [1.5 + 0.5, 1.5 + 0.5 + 0.2]; % these trials were early lick aborts by trains, we will exclude if there was a rxn lick because could still be a rxn or train here, and this will eliminate trains
operant_no_rew_range = [1.5 + 0.5 + buffer_time, 1.5 + 3.333]; % this is window of what we score as 1st lick in No-lick window till the target + 0.1 because is too fast for rxn
operant_rew_range = [1.5 + 3.333, 1.5 + target_time+.1]; %1.5 + 3.5]
pavlovian_range = [1.5 + target_time+.1, 1.5 + 7];
