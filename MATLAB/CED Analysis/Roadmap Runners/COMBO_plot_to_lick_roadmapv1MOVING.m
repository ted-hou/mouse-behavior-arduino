...
%% EARLY and REW CASE:---------------------------------------------
	...
	for ibins = 1:nbins
		...
		plot((time_array(pos1:pos2)), smooth(DLS_bin_aves{ibins}(pos1:pos2), smooth_kernel, 'moving'), 'linewidth', 1);
		hold on;
	...
	plot([xwin], [0,0], 'k-', 'linewidth', 1)
	for ibins = 1:nbins
		plot(time_array(pos1:pos2), smooth(SNc_bin_aves{ibins}(pos1:pos2), smooth_kernel, 'moving'), 'linewidth', 1);
		hold on;
		% names{ibins+2} = ['Bin # ', num2str(ibins)];
	end
	legend(names);
