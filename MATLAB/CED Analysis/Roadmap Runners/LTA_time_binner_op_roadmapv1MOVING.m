...
% 	last modified 8-01-17 ahamilos
% 
% 	Dependencies:
...
%% RXN CASE:---------------------------------------------
	...
	for ibins = 1:nbins
		plot(time_array(pos1:pos2), smooth(DLS_bin_aves{ibins}(pos1:pos2), 500, 'moving'), 'linewidth', 3);
	...
	plot([xwin], [0,0], 'k-', 'linewidth', 3)
	for ibins = 1:nbins
		plot(time_array(pos1:pos2), smooth(SNc_bin_aves{ibins}(pos1:pos2), 500, 'moving'), 'linewidth', 3);
		% names{ibins+2} = ['Bin # ', num2str(ibins)];
	end
	legend(names);
	xlim(xwin)
