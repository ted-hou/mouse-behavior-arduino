...
% 
% 
smooth_kernel = 200; % note is now moving
nbins = 3;
cue_on_time = 1500;
...
all_ex_first_licks = combined_data_struct.all_ex_first_licks;
...
early_DLS_lick_triggered_trials = combined_data_struct.early_DLS_lick_triggered_trials;
rew_DLS_lick_triggered_trials = combined_data_struct.rew_DLS_lick_triggered_trials;
early_SNc_lick_triggered_trials = combined_data_struct.early_SNc_lick_triggered_trials;
rew_SNc_lick_triggered_trials = combined_data_struct.rew_SNc_lick_triggered_trials;
...
[f_ex_licks_with_rxn, f_ex_licks_no_rxn] = rxn_lick_or_no_rxn_lick_fx(combined_data_struct.all_ex_first_licks, combined_data_struct.f_ex_lick_rxn);
...
% Clip data to up to lick time---------------------------------------------------------------
	...
	for i_trial = 1:length(combined_data_struct.all_ex_first_licks)
		if combined_data_struct.all_ex_first_licks(i_trial) == 0
...
%% COMBINED (ALL OPERANTS) CASE--------PLUS LICK----------------------------------------------------------------------------------
	...
	binstart = time_bound_1_early + half_point;
	binendnum = time_bound_1_early + half_point;
	...
	for ibins = 1:nbins
		...
		plot([1:size(fxn, 2)] - 1500, smooth(fxn, smooth_kernel, 'moving'), 'linewidth', 1);
	end
	legend(names);
	ylim([-1,1])
...
% 		DLS_holder_min = smooth(DLS_holder_min, smooth_kernel, 'moving');
% 		SNc_holder_min = SNc_bin_aves{ibins}(1500:end);
% 		SNc_holder_min = smooth(SNc_holder_min, smooth_kernel, 'moving');
		
% 		DLS_holder_max = DLS_bin_aves{ibins}(2000:end);
% 		DLS_holder_max = smooth(DLS_holder_max, smooth_kernel, 'moving');
% 		SNc_holder_max = SNc_bin_aves{ibins}(2000:end);
% 		SNc_holder_max = smooth(SNc_holder_max, smooth_kernel, 'moving');
...
%% EARLY and REW CASE:---------------------------------------------
	...
	for ibins = 1:nbins
		...
		plot((time_array(pos1:pos2)), smooth(DLS_bin_aves{ibins}(pos1:pos2), 50, 'moving'), 'linewidth', 1);
		hold on;
	...
	plot([xwin], [0,0], 'k-', 'linewidth', 1)
	for ibins = 1:nbins
		plot(time_array(pos1:pos2), smooth(SNc_bin_aves{ibins}(pos1:pos2), 50, 'moving'), 'linewidth', 1);
		hold on;
		% names{ibins+2} = ['Bin # ', num2str(ibins)];
	end
	legend(names);
