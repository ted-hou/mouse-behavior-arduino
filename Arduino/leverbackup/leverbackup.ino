...
void loop()
{
	...
	while (true)
	{
		...
		// if (signedMillis() - _debugTimer == 1000)  {       // If 1000 ms since last tick readout...
		...
		//   _debugTimer = signedMillis();                          // reset debug clock
		// }
		// else if (signedMillis() - _debugTimer > 1000)  {   // If MORE than 1000 ms have passed, it means we lost a ms on the last loop
		//   if (_params[_DEBUG]) {
		...
		//   _debugTimer = signedMillis();                          // reset debug clock
		// }
		// else if (signedMillis() - _debugTimer < 1000)  {   // If LESS than 1000 ms have passed, acculumate a loop tick
...
void init_trial() {
	...
	if (getLeverState()) {                            // MOUSE: "Lever Pressed"
		if (!_lever_state) {                             // If a new press initiated
			...
			//------------------------DEBUG MODE--------------------------//
				if (_params[_DEBUG]) {sendMessage("Lever pressed to init trial");}
			//----------------------end DEBUG MODE------------------------//
...
void random_delay() {
	...
	if (_state != _prevState) {                       // If ENTERTING RANDOM DELAY STATE:
		//-----------------INIT TRIAL CLOCKS and OUTPUTS--------------//
		sendMessage("&" + String(EVENT_RANDOM_DELAY) + " " + String(signedMillis() - _exp_timer));
		_prevState = _state;                                // Assign _prevState to READY _state
	...
	if (getLickState()) {                            // MOUSE: "Licked"
		if (!_lick_state) {                              // If a new lick initiated
			...
			_lick_state = true;                            // Halts lick detection
			//------------------------DEBUG MODE--------------------------//
			if (_params[_DEBUG]) {sendMessage("Pre-cue lick detected, tallying lick @ " + String(signedMillis() - _random_delay_timer - _preCueDelay) + "ms wrt Cue ON");}
			//----------------------end DEBUG MODE------------------------//
		}
	}
...
void pre_window() { 
	...
	long current_time = signedMillis()-_cue_on_time; 
...
void response_window() {
	...
	if (!getLeverState()) {                           // MOUSE: "No press"
		if (_lever_state) {                               // If press just ended  
			...
			sendMessage("&" + String(EVENT_CORRECT_RELEASE) + " " + String(signedMillis() - _exp_timer));
			sendMessage("&" + String(EVENT_RELEVANT_RELEASE) + " " + String(signedMillis() - _exp_timer));
	...
	if (getLickState()) {                            // MOUSE: "Licked"
		if (!_lick_state) {                              // If new lick
			...
			//------------------------DEBUG MODE--------------------------//
			if (_params[_DEBUG]) {
				_lick_time = signedMillis() - _cue_on_time;          // Records lick wrt cue onset
				sendMessage("Response Window Early lick detected @ " + String(_lick_time) + "ms wrt Cue Onset.");
			}
			//----------------------end DEBUG MODE------------------------//

			//~~~~~~~~~~~~~~~~~~~~~~~~~Deliver Quinine if Window Open~~~~~~~~~~~~~~~~~~~~~~~~//
			if (signedMillis() - _cue_on_time > _params[QUININE_MIN] && signedMillis()-_cue_on_time < _params[QUININE_MAX]) { // If quinine window open
				_quinine_timer = signedMillis();                     // Records time of last quinine delivery (we know this is first in this trial since the Action List only runs 1x)
...
void reward() {
	...
	if (!_lick_state) {                              // If a new lick initiated
		...
		//------------------------DEBUG MODE--------------------------//
		if (_params[_DEBUG]) {
		sendMessage("Lick detected, tallying lick @ " + String(signedMillis() - _cue_on_time) + "ms");
		}
		//----------------------end DEBUG MODE------------------------//
	}
	}
