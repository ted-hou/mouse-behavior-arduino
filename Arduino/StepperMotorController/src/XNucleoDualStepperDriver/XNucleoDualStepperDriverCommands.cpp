...
//  the dSPIN chip.
void XNucleoStepper::setParam(byte param, unsigned long value)
{
  param |= SET_PARAM;
  SPIXfer((byte)param);
...
//  the dSPIN chip.
long XNucleoStepper::getParam(byte param)
{
...
void XNucleoStepper::run(byte dir, float stepsPerSec)
{
  SPIXfer(RUN | dir);
...
void XNucleoStepper::stepClock(byte dir)
{
...
void XNucleoStepper::move(byte dir, unsigned long numSteps)
{
  SPIXfer(MOVE | dir);
...
void XNucleoStepper::goTo(long pos)
{
  SPIXfer(GOTO);
...
void XNucleoStepper::goToDir(byte dir, long pos)
{
...
void XNucleoStepper::goUntil(byte action, byte dir, float stepsPerSec)
{
...
//  for act.
void XNucleoStepper::releaseSw(byte action, byte dir)
{
...
//  ABS_POS values.
void XNucleoStepper::setMark(long newMark)
{
  setParam(MARK, newMark);
}

void XNucleoStepper::setPos(long newPos)
{
...
String XNucleoStepper::getFullStatusString() {
  String statusString = "Status: ";
  uint16_t status = getStatus();
  if (status & STATUS_SCK_MOD) {statusString += "SCK_MOD ";}
  if (!(status & STATUS_STEP_LOSS_B)) {statusString += "STEP_LOSS_B ";}
  if (!(status & STATUS_STEP_LOSS_A)) {statusString += "STEP_LOSS_A ";}
  if (!(status & STATUS_OCD)) {statusString += "OverCurrent-Detection ";} // OCD
  if (!(status & STATUS_TH_SD)) {statusString += "Thermal-Shutdown ";} // TH_SD
  if (!(status & STATUS_TH_WRN)) {statusString += "Thermal-Warning ";} // TH_WRN
  if (!(status & STATUS_UVLO)) {statusString += "Undervoltage-Lockout ";} // UVLO
  if (status & STATUS_WRONG_CMD) {statusString += "Wrong-Command ";} // WRONG_CMD
  if (status & STATUS_NOTPERF_CMD) {statusString += "NotPerformed-Command ";} // NOTPERF_CMD
  if (status & STATUS_DIR) {statusString += "FWD ";} else {statusString += "REV ";}// DIR
  if (status & STATUS_SW_EVN) {statusString += "Switch-Event ";} //SW_EVN
  if (status & STATUS_SW_F) {statusString += "Switch-Closed ";} // SW_F
  if (!(status & STATUS_BUSY)) {statusString += "BUSY ";}
  if (status & STATUS_HIZ) {statusString += "Hi-Z ";}
  uint16_t mot_status = status & STATUS_MOT_STATUS;
  if (mot_status == STATUS_MOT_STATUS_ACCELERATION) { // MOT_STATUS
    statusString += "Accelerating ";
  } else if (mot_status == STATUS_MOT_STATUS_DECELERATION) {
    statusString += "Deccelerating ";
  } else if (mot_status == STATUS_MOT_STATUS_CONST_SPD) {
    statusString += "Constant-Speed ";
  } else {
...
String XNucleoStepper::getAlarmStatusString() {
  String statusString = "Alarm: ";
  uint16_t status = getStatus();
  if (!(status & STATUS_STEP_LOSS_B)) {statusString += "STEP_LOSS_B ";}
  if (!(status & STATUS_STEP_LOSS_A)) {statusString += "STEP_LOSS_A ";}
  if (!(status & STATUS_OCD)) {statusString += "OverCurrent-Detection ";} // OCD
...
//   String statusString = "Status: ";
//   uint16_t status = getStatus();
//   if (!(status & (1<<14))) {statusString += "STEP_LOSS_B ";}
//   if (!(status & (1<<13))) {statusString += "STEP_LOSS_A ";}
//   if (!(status & (1<<12))) {statusString += "OverCurrent-Detection ";} // OCD
//   if (!(status & (1<<11))) {statusString += "Thermal-Shutdown ";} // TH_SD
//   if (!(status & (1<<10))) {statusString += "Thermal-Warning ";} // TH_WRN
//   if (!(status & (1<<9))) {statusString += "Undervoltage-Lockout ";} // UVLO
//   if (status & (1<<8)) {statusString += "Wrong-Command ";} // WRONG_CMD
//   if (status & (1<<7)) {statusString += "NotPerformed-Command ";} // NOTPERF_CMD
//   if (status & (1<<3)) {statusString += "Switch-Event ";} //SW_EVN
//   return statusString;
// }
