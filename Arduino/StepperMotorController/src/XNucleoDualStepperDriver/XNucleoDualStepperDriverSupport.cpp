...
// XNucleoStepperSupport.cpp - Contains utility functions for converting real-world
//  units (eg, steps/s) to values usable by the dsPIN controller. These are all
//  private members of class XNucleoStepper.
...
unsigned long XNucleoStepper::accCalc(float stepsPerSecPerSec)
{
...
float XNucleoStepper::accParse(unsigned long stepsPerSecPerSec)
{
...
// This is a 12-bit value, so we need to make sure the value is at or below 0xFFF.
unsigned long XNucleoStepper::decCalc(float stepsPerSecPerSec)
{
  float temp = stepsPerSecPerSec * 0.137438;
  if( (unsigned long) long(temp) > 0x00000FFF) return 0x00000FFF;
  else return (unsigned long) long(temp);
...
float XNucleoStepper::decParse(unsigned long stepsPerSecPerSec)
{
    return (float) (stepsPerSecPerSec & 0x00000FFF) / 0.137438;
}
...
unsigned long XNucleoStepper::maxSpdCalc(float stepsPerSec)
{
...
float XNucleoStepper::maxSpdParse(unsigned long stepsPerSec)
{
...
// This is a 12-bit value, so we need to make sure the value is at or below 0xFFF.
unsigned long XNucleoStepper::minSpdCalc(float stepsPerSec)
{
...
float XNucleoStepper::minSpdParse(unsigned long stepsPerSec)
{
...
unsigned long XNucleoStepper::FSCalc(float stepsPerSec)
{
...
float XNucleoStepper::FSParse(unsigned long stepsPerSec)
{
...
unsigned long XNucleoStepper::intSpdCalc(float stepsPerSec)
{
...
float XNucleoStepper::intSpdParse(unsigned long stepsPerSec)
{
...
unsigned long XNucleoStepper::spdCalc(float stepsPerSec)
{
...
float XNucleoStepper::spdParse(unsigned long stepsPerSec)
{
...
long XNucleoStepper::paramHandler(byte param, unsigned long value)
{
...
long XNucleoStepper::xferParam(unsigned long value, byte bitLen)
{
...
byte XNucleoStepper::SPIXfer(byte data)
{
  byte dataPacket[_motorsPerBoard];
  int i;
  for (i=0; i < _motorsPerBoard; i++)
  {
    dataPacket[i] = 0;
  }
  ...
  _SPI->transfer(dataPacket, _motorsPerBoard);
  _SPI->endTransaction();
