...
void XNucleoStepper::configSyncPin(byte pinFunc, byte syncSteps)
{
...
void XNucleoStepper::configStepMode(byte stepMode)
{
...
float XNucleoStepper::getMaxSpeed()
{
...
void XNucleoStepper::setMinSpeed(float stepsPerSecond)
{
  // We need to convert the floating point stepsPerSecond into a value that
  //  the dSPIN can understand. Fortunately, we have a function to do that.
...
float XNucleoStepper::getMinSpeed()
{
...
//  mode. 
void XNucleoStepper::setFullSpeed(float stepsPerSecond)
{
  unsigned long integerSpeed = FSCalc(stepsPerSecond);
  setParam(FS_SPD, integerSpeed);
}

float XNucleoStepper::getFullSpeed()
{
...
void XNucleoStepper::setPWMFreq(int divisor, int multiplier)
{
...
int XNucleoStepper::getPWMFreqDivisor()
{
  return (int) (getParam(CONFIG) & 0xE000);
}

int XNucleoStepper::getPWMFreqMultiplier()
{
  return (int) (getParam(CONFIG) & 0x1C00);
}

// Slew rate of the output in V/us. Can be 180, 290, or 530.
void XNucleoStepper::setSlewRate(int slewRate)
{
  unsigned long configVal = getParam(CONFIG);
...
int XNucleoStepper::getOCShutdown()
{
...
void XNucleoStepper::setVoltageComp(int vsCompMode)
{
  unsigned long configVal = getParam(CONFIG);
...
int XNucleoStepper::getVoltageComp()
{
...
void XNucleoStepper::setSwitchMode(int switchMode)
{
  unsigned long configVal = getParam(CONFIG);
...
int XNucleoStepper::getSwitchMode()
{
...
void XNucleoStepper::setOscMode(int oscillatorMode)
{
  unsigned long configVal = getParam(CONFIG);
...
void XNucleoStepper::setAccKVAL(byte kvalInput)
{
  setParam(KVAL_ACC, kvalInput);
}

byte XNucleoStepper::getAccKVAL()
{
  return (byte) getParam(KVAL_ACC);
}

void XNucleoStepper::setDecKVAL(byte kvalInput)
{
  setParam(KVAL_DEC, kvalInput);
}

byte XNucleoStepper::getDecKVAL()
{
  return (byte) getParam(KVAL_DEC);
}

void XNucleoStepper::setRunKVAL(byte kvalInput)
{
  setParam(KVAL_RUN, kvalInput);
}

byte XNucleoStepper::getRunKVAL()
{
  return (byte) getParam(KVAL_RUN);
}

void XNucleoStepper::setHoldKVAL(byte kvalInput)
{
  setParam(KVAL_HOLD, kvalInput);
}

byte XNucleoStepper::getHoldKVAL()
{
...
void XNucleoStepper::setLoSpdOpt(boolean enable)
{
...
boolean XNucleoStepper::getLoSpdOpt()
{
