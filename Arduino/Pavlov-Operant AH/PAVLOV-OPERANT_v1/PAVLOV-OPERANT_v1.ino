/*********************************************************************
  ...
  Last Modified 9/16/16 - ahamilos
...
*****************************************************/
  ...
  *****************************************************/
    ...
    static unsigned long _pre_window_timer = 0;      // Tracks time in pre_window state
...
*****************************************************/
  ...
  void init_trial() {
    ...
    if (_state != _prevState) {                       // If ENTERTING READY STATE:
      ...
      //------------------------DEBUG MODE--------------------------//
        if (_params[_DEBUG]) {
          sendMessage("Trial Started. Random Pre-Cue Delay in Progress...(" + String(preCueDelay) + "ms");
        }
      //----------------------end DEBUG MODE------------------------//
    }
    ...
    if (millis() - _random_delay_timer >= preCueDelay) {// Pre-Cue Delay elapsed -> PRE_WINDOW
      //------------------------DEBUG MODE--------------------------//  
  ...
  void post_window() {
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      ...
      if (millis() - _trialTimer >= _params[TRIAL_DURATION]) {  // TRIAL END -> ITI
  ...
  void abort_trial() {
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      ...
      if (millis() - _trialTimer >= _params[TRIAL_DURATION]) { // Trial Timeout -> ITI
        // Send event marker (trial end) to HOST with timestamp relative to trial start
        sendMessage("&" + String(EVENT_TRIAL_END) + String(millis() - _trialTimer));
        _state = INTERTRIAL;                                      // Move to ITI
        return                                                    // Exit Fx
