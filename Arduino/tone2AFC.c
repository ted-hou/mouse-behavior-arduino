...
enum EventMarker
{
	...
	EVENT_LICK_R,					// Lick onset
	EVENT_LICK_L,					// Lick onset
	EVENT_LICK_OFF,					// Lick offset
	EVENT_STIM_ON,					// Stim on
	EVENT_STIM_OFF,					// Stim off
	EVENT_DELAY_START,				// Delay start
	EVENT_DELAY_END,				// Delay end
...
{
	// Init pins
	pinMode(PIN_REWARD_L, OUTPUT);		// Reward, set to HIGH to open juice valve
...
void state_stim_on() 
{
	...
	*****************************************************/
	...
	{
		sendEventMarker(EVENT_STIM_OFF, -1);
		_state = STATE_DELAY;
		return;
	}
...
void state_delay() 
{
	...
	*****************************************************/
	if (_state != _prevState) 
	{
		...
		sendEventMarker(EVENT_DELAY_START, -1);
	}
	...
	*****************************************************/
	...
	// _delay elapsed --> GO_CUE
	if getTimeSinceStimOn() >= _params[DELAY_DURATION]
	{
		sendEventMarker(EVENT_DELAY_END, -1);
		_state = STATE_GO_CUE;
		return;
	}
...
void state_go_cue() 
{
	...
	*****************************************************/
	if (_state != _prevState) 
	{
		...
		sendEventMarker(EVENT_GO_CUE_ON, -1);
	}
...
void state_response_window() 
{
	...
	*****************************************************/
	...
	// Correct lick --> REWARD
	// need to figure out if i have to separate out the two _isLickOnset
	// i think yes which is very annoying
...
void handleLickR() 
{
	if (getLickStateR() && !_isLicking)
	{
		...
		sendEventMarker(EVENT_LICK_R, -1);
	}
	else
	{
		if (!getLickStateR() && _isLicking)
...
void handleLickL() 
{
	if (getLickStateL() && !_isLicking)
	{
		...
		_isLickOnset = true;
		_timeLastLick = getTime();
		sendEventMarker(EVENT_LICK_L, -1);
	}
	else
	{
		if (!getLickStateL() && _isLicking)
...
void setRewardR(bool turnOn) 
{
	static bool rewardOn = false;
	if (turnOn)
	{
		digitalWrite(PIN_REWARD_R, HIGH);
		if (!rewardOn)
		{
			rewardOn = true;
...
// Toggle juice valve, register event when state is changed
void setRewardL(bool turnOn) 
{
	static bool rewardOn = false;
	if (turnOn)
	{
		digitalWrite(PIN_REWARD_L, HIGH);
		if (!rewardOn)
		{
			rewardOn = true;
			sendEventMarker(EVENT_REWARD_ON, -1);
