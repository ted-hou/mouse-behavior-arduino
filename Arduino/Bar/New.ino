...
enum State
{
	...
	STATE_ABORT_STIM_ON,		// Early lick during stat
...
enum EventMarker
{
	...
	EVENT_STIM_MOVE,				// Stim begins moving
...
enum ResultCode
{
	...
	CODE_OPERANT,			// Operant (reward given if animal licks/presses to make stim reverse)
	CODE_MOVE_REWARD,		// If animal presses/licks, rewarded
	CODE_PAVLOVIAN,			// Pavlovian (Reward given when stim reverses)
...
long _params[_NUM_PARAMS] = 
{
	...
	1,		// USE_LEVER
	1,		// ALLOW_PRESS_STIM_ON
	0,		// ALLOW_EARLY_PRESS
	1,		// ALLOW_LICK_STIM_ON
	0,		// ALLOW_EARLY_LICK
...
/*****************************************************
	STIM_ON - Moving dots, stationary stim
*****************************************************/
void state_stim_on() 
{
	/*****************************************************
	...
	*****************************************************/
	...
	// Early lick/lever-press detected --> ABORT
	if ((_isLickOnset && (_params[ALLOW_LICK_STIM_ON] == 0)) || (_isLeverPressOnset && (_params[ALLOW_PRESS_STIM_ON] == 0)))
	{
		// Register result
		_resultCode = CODE_EARLY_MOVE;
		_state = STATE_ABORT_STIM_ON;
		return;	
	}

	// stim_on elapsed --> STIM_MOVE
	if (getTimeSinceStimOn() >= _params[STIM_ON_DURATION] && (getTimeSinceLastLick() >= _params[MOVE_TIMEOUT]) && (getTimeSinceLastPress() >= _params[MOVE_TIMEOUT]))
	{
		_state = STATE_STIM_MOVE;
		return;
	}
...
/*****************************************************
	STIM_MOVE - Moving dots, moving stim
*****************************************************/
void state_stim_move() 
{
	/*****************************************************
		ACTION LIST
	*****************************************************/
	if (_state != _prevState) 
	{
		...
		sendEventMarker(EVENT_STIM_MOVE, -1);
	}
	...
	*****************************************************/
	...
	// stim_move elapsed --> RESPONSE_WINDOW
...
/*****************************************************
	ABORT - Early lick during stim_on
*****************************************************/
void state_abort_stim_on()
{
	/*****************************************************
		ACTION LIST
	*****************************************************/
